...
\section{実行環境・使用技術}
\begin{itemize}
  \item macOSBigSur version 11.2.3
  \item Python 3.10.8
\end{itemize}
...
\footnote{DiscordのBotの作り方(メモ)https://note.com/exteoi/n/nf1c37cb26c41 (参照2023.3.29)}。
...
\footnote{discord.pyドキュメントhttps://discordpy.readthedocs.io/ja/latest/intro.html#basic-concepts (参照2023.3.29)}
...
main.pyのimport discordとclass MyClientの間に以下のコードを追加して下さい。
\begin{tcolorbox}[breakable]
\begin{verbatim}
1 import os
...
Botに機能をつけるには上記のソースコードの8行目と10行目の間にコードを付け足していきます。\\
上記のソースコードの8行目と10行目の間に下記のコードを付け足すことで、Botは特定のワードに反応して、特定のメッセージを返答するようになります。
\begin{tcolorbox}[breakable]
\begin{verbatim}
...
「仕事終わった」や「お疲れ様」に当たる部分は各自で好きな言葉に設定して楽しんで下さい。